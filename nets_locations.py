...
file_to_open = os.path.join(data_folder_read, "NETS2015_general_with_empty_FIPS.csv")
file_to_write_partial = os.path.join(data_folder_write, "NETS2015_general_with_FIPS_partial.csv")
file_to_write = os.path.join(data_folder_write, "NETS2015_general_with_FIPS_2.csv")
...
batch_len = 1000 # A batch refers to one set of URLs to be pulled at once
...
while current_location_in_full_df <= len(df):
    fips_list = []
    ...
    #print('incrementing')
    current_location_in_full_df += 1000
    if current_location_in_full_df % 300000 == 0:
